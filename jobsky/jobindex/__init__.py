...
log = create_logger("JobIndex")


class JobIndex(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        try:
            ...
            if resp.status_code != 200:
                log.error(f"JobIndex HTTP {resp.status_code}")
            ...
            for a in soup.find_all("a", href=True):
                ...
                if "/job/" in href or "/jobs/" in href:
                    ...
                    jobs.append(JobPost(id=f"jobindex-{hash(url)}", title=title, job_url=url))
                    if len(jobs) >= scraper_input.results_wanted:
                        break
        except Exception as e:
            log.error(f"JobIndex scrape error: {e}")
