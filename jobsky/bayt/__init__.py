...
from jobsky.util import create_logger, create_session
...
class BaytScraper(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        self.session = create_session(
            proxies=self.proxies, ca_cert=self.ca_cert, is_tls=False, has_retry=True, user_agent=self.user_agent
        )
        # Ensure session headers include a User-Agent if provided
        if self.user_agent:
            try:
