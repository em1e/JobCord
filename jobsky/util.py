...
from jobsky.model import CompensationInterval, JobType, Site
...
class TLSRotating(RotatingProxySession, tls_client.Session):
    ...
    def execute_request(self, *args, **kwargs):
        # Map common requests-style kwargs to tls_client expected names
        # tls_client.Session.execute_request expects `timeout_seconds` instead of `timeout`
        if "timeout" in kwargs and "timeout_seconds" not in kwargs:
            # translate `timeout` (seconds float/int) to `timeout_seconds`
...
def create_session(
    *,
    ...
    # Set a default User-Agent if provided to help avoid simple bot blocks
    if user_agent:
        try:
            session.headers.update({"User-Agent": user_agent})
        except Exception:
            # Some session types may not support headers update at creation; ignore safely
            pass
