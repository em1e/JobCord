...
log = create_logger("JobsInFinland")


class JobsInFinland(Scraper):
    base_url = "https://www.jobsinfinland.fi"
    search_url = "https://www.jobsinfinland.fi/jobs"
    delay = 1
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        try:
            ...
            if resp.status_code != 200:
                log.error(f"JobsInFinland HTTP {resp.status_code}")
                return JobResponse(jobs=[])

            soup = BeautifulSoup(resp.text, "html.parser")
            for a in soup.find_all("a", href=True):
                href = a["href"]
                if "/job/" in href or "/vacancies/" in href or "job" in href:
                    url = href if href.startswith("http") else urljoin(self.base_url, href)
                    title = a.get_text(strip=True) or "N/A"
                    jobs.append(JobPost(id=f"jobsinfinland-{hash(url)}", title=title, job_url=url))
                    if len(jobs) >= scraper_input.results_wanted:
                        break
        except Exception as e:
            log.error(f"JobsInFinland scrape error: {e}")
