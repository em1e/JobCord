...
log = create_logger("Mol")


class Mol(Scraper):
    base_url = "https://mol.fi"
    # TE-palvelut / mol.fi uses search endpoints; we use a generic path as a starting point
    search_url = "https://mol.fi/tyopaikat"
    delay = 1
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        try:
            ...
            if resp.status_code != 200:
                log.error(f"Mol HTTP {resp.status_code}")
                return JobResponse(jobs=[])

            soup = BeautifulSoup(resp.text, "html.parser")
            for a in soup.find_all("a", href=True):
                href = a["href"]
                if "/tyopaikat/" in href or "/job/" in href:
                    url = href if href.startswith("http") else urljoin(self.base_url, href)
                    title = a.get_text(strip=True) or "N/A"
                    jobs.append(JobPost(id=f"mol-{hash(url)}", title=title, job_url=url))
                    if len(jobs) >= scraper_input.results_wanted:
                        break
        except Exception as e:
            log.error(f"Mol scrape error: {e}")
