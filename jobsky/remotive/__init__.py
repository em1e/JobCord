...
class Remotive(Scraper):
    ...
    search_url = "https://remotive.io/remote-jobs/search"
    delay = 1
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        jobs: List[JobPost] = []
        try:
            params = {"search": scraper_input.search_term} if getattr(scraper_input, "search_term", None) else {}
            resp = self.session.get(self.search_url, params=params, timeout=getattr(scraper_input, "request_timeout", 30))
            if resp.status_code != 200:
                log.error(f"Remotive HTTP {resp.status_code}")
            ...
            for a in soup.find_all("a", href=True):
                ...
                if "/remote-jobs/" in href or "/job/" in href:
                    ...
                    jobs.append(JobPost(id=f"remotive-{hash(url)}", title=title, job_url=url))
                    if len(jobs) >= scraper_input.results_wanted:
                        break
        except Exception as e:
            log.error(f"Remotive scrape error: {e}")
