...
from jobsky.ziprecruiter.constant import headers, get_cookie_data
...
from jobsky.ziprecruiter.util import get_job_type_enum, add_params
...
class ZipRecruiter(Scraper):
    ...
    def __init__(
        ...
        """
        super().__init__(Site.ZIP_RECRUITER, proxies=proxies, ca_cert=ca_cert, user_agent=user_agent)

        self.scraper_input = None
        self.session = create_session(proxies=proxies, ca_cert=ca_cert, user_agent=self.user_agent)
        # ensure user agent is present in the headers used for requests
        try:
            headers_local = headers.copy()
            if self.user_agent:
                headers_local["User-Agent"] = self.user_agent
            self.session.headers.update(headers_local)
        except Exception:
            # fallback to best-effort header update
            try:
